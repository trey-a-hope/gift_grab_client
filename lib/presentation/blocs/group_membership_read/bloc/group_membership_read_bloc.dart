...
class GroupMembershipReadBloc
  ...
  GroupMembershipReadBloc(String groupId, this.client, this.sessionService)
  ...
  Future<void> _onReadGroupMembershipState(
    ...
    action: () async {
      ...
      final groupMembershipState = await _getGroupMembershipState(
        session,
  ...
  Future<GroupMembershipState?> _getGroupMembershipState(
    Session session, {
    String? groupId,
  }) async {
    if (groupId == null) return null;

    final val = await client.rpc(
      session: session,
      id: RpcFunctions.GET_GROUP_MEMBERSHIP_STATE.id,
      payload: jsonEncode({"group_id": groupId}),
    );
